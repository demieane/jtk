...
def getLogs(curves):
  nc = len(curves)  # find the number of well log types

  fileName = "data/"+curves[0]+"logs.txt"
  ifile = open(fileName,'r+')
  lines = ifile.readlines()
  nz = len(lines)  # number of lines
  ifile.close()
  ifile = open(fileName,'r+')
  c = 0
  start = False
  while (start!=True):
    ...
    if line == "":
      ...
      start = True
  # It waits  for the ~ to skip this line and start reading data
  line  = ifile.readline()
  wdata = line.split('\t')
  nl = len(wdata)-1  # data from logs are one less because of the depth parameter
  ...
  for ic,cv in enumerate(curves):
    ...
    line = ifile.readline()
    wdata = line.split('\t')
    depth[0] = float(wdata[0])  # The first column should be the depth in [m]
    for l in range(1,nl):
