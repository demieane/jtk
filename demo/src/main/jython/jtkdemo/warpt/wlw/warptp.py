...
curves  = ["v", "p", "d"]
weights = [1.0, 1.0, 2.0]
epow    = [0.25,0.25,0.125]
"""
curves  = ["v", "d",  "p", "g"]
weights = [1.0, 2.0,  1.0, 0.0]
epow    = [0.25,0.125,0.125,0.05]
"""
wells = None
logs = None
fnull = -999.2500
...
def goXSection():
  ...
  for index,well in enumerate(wells):
    x.append(well.x2[0])
    y.append(well.x3[0])
  x = [x[141],x[ 80],x[124],x[125],x[130],x[159]]
  y = [y[141],y[ 80],y[124],y[125],y[130],y[159]]
  
  sz,wl = resampleMulti(wells,curves)
  wl = [wl[141],wl[ 80],wl[124],wl[125],wl[130],wl[159]] # xsection
  nl = len(wl)
  nz = len(wl[0][0])
  dmin = FLT_MAX
  for i in range(1,nl):
  ...
  for i in range(1,nl):
    d = dist(x[i],y[i],x[i-1],y[i-1])
    nx += int(floor(d/dmin+0.5))
  sx = Sampling(nx,dmin,0.0) 
  fs = fillfloat(fnull,nz,nx)
  fs[0] = wl[0][2] # density
  nx = 1
  for i in range(1,nl):
    d = dist(x[i],y[i],x[i-1],y[i-1])
  ...
  for i,c in enumerate(curves):
    ...
    fmed = wlw.replaceWithMedian(gs)
    fmad = wlw.replaceWithMAD(gs)
    mdev = wlw.medianAbsoluteDeviation(gs)
    print c,"median absolute deviation: ",mdev
    mdevs += mdev
    fs = wlw.replaceNulls(fs,freplace)
...
def goFlip():
  ...
  nl = len(wl)
  #normalizeByMedian("g",curves,wl)
  #normalizeByMedian("d",curves,wl)
  ...
  s = wlw.findShifts(weights, wl)
  print "flipped"
  stest = wlw.findShifts(weights,wtest)
  st = copy(stest)
  for i in range(nl):
    for k in range(nk):
      st[i][k] = stest[i][nk-1-k]
  st = mul(-1,st)
  #s = wlw.findShiftsM(wl)
  fs = zerofloat(len(wl[0][0]),nl)
  fw = zerofloat(len(wl[0][0]),nl)
  freplace = 2.0
  fclips = (2.0,6.0)
  cblabel = "Velocity (km/s)"
  s = mul(1000*sz.delta,s) # convert shifts to m
  for i,c in enumerate(curves):
    for j in range(nl):
      ...
      fw[j] = wtest[j][i]
      gs = wlw.applyShifts(fs,s)
      gstest = wlw.applyShifts(fs,st)
      #gstest = wlw.applyShifts(fw,stest)
      if c=="d":
        freplace = 1.0
        fclips = (2.0,2.8)
        cblabel = "Density (g/cc)"
      ...
      if c=="g":
        ...
        fclips = (30.0,160.0)
        cblabel = "Gamma ray (API)"
    gtest = zerofloat(nk,nl)
    for l in range(nl):
      for k in range(nk):
        gtest[l][k] = gstest[l][nk-1-k]
    #gs = copy(gtest)
    fs = wlw.replaceNulls(fs,freplace)
    gs = wlw.replaceNulls(gs,freplace)
    sp = SimplePlot(SimplePlot.Origin.UPPER_LEFT)
    ...
    sp.addColorBar(cblabel)
    sp.plotPanel.setColorBarWidthMinimum(90)
    pv = sp.addPixels(sz,Sampling(nl,1,1),gstest)
    pv.setInterpolation(PixelsView.Interpolation.NEAREST)
    pv.setColorModel(cjet)
    pv.setClips(fclips[0],fclips[1])
    #sp.setFontSizeForPrint(12.0,222) #GP newsletter
  ...
  sp1.addColorBar("Shifts (RGT)")
  sp1.plotPanel.setColorBarWidthMinimum(90)
  pv = sp1.addPixels(sz,Sampling(nl,1,1),st)
  pv.setClips(-250,250)
  pv.setInterpolation(PixelsView.Interpolation.NEAREST)
  pv.setColorModel(cjet)
...
def goShifts():
  sz,wl = resampleMulti(wells,curves)
  #wl = [wl[4],wl[14],wl[17],wl[20]] # deepest 7 velocity logs
  #wl = [wl[0],wl[4],wl[9],wl[11],wl[14],wl[17],wl[20]] # deepest 7 velocity logs
  ...
  #      wl[159],wl[168]]
  #wl = [wl[  1],wl[  5],wl[ 11],wl[ 12],wl[ 13],
  #      wl[ 21],wl[ 25],wl[ 26],wl[ 55],wl[ 62],
  #      wl[ 74],wl[ 78],wl[ 80],wl[ 82],wl[ 85],
  #      wl[ 86],wl[ 94],wl[100],wl[120],wl[127],
  #      wl[129],wl[134],wl[138],wl[141],wl[142],
  #      wl[145],wl[146],wl[151],wl[152]] # all v with p & d
  #wl = [wl[  5],wl[ 11],wl[ 13],wl[ 21],
  #      wl[ 62],wl[ 78],wl[ 80],wl[ 82],wl[ 85],
  #      wl[ 86],wl[ 94],wl[100],wl[120],wl[127],
  #      wl[134],wl[138],wl[145],wl[151],wl[152]] # all v,p,d
  #wl = [wl[ 80],wl[ 90],wl[ 94],wl[104],wl[107],
  #      wl[122],wl[123],wl[124],wl[125],wl[127],
  #      wl[130],wl[141],wl[147],wl[149],wl[150],
  #      wl[152],wl[159],wl[168]] # deepest 18 density logs for v,p,d
  #wl = [wl[  0],wl[  1],wl[  2],wl[  5],wl[  6],
  #      wl[ 10],wl[ 23],wl[ 36],wl[ 44],wl[ 52],
  #      wl[116],wl[119],wl[126],wl[134],wl[138],
  #      wl[145],wl[147],wl[164],wl[165],wl[170],
  #      wl[172]] 
  #wl = [wl[  0],wl[  1],wl[  2],wl[  5],wl[  6],
  #      wl[  8],wl[ 10],wl[116],wl[119],wl[134],
  #      wl[145],wl[170],wl[172]] 
        #wl[145],wl[164],wl[165],wl[170],wl[172]] 
  wl = [wl[ 80],wl[ 94],wl[124],wl[125],wl[127],
        wl[130],wl[141],wl[147],wl[149],wl[150],
        wl[152],wl[159],wl[168]] # thesis figs
  #wl = [wl[141],wl[ 80],wl[124],wl[125],wl[130],wl[159]] # xsection
  nl = len(wl)
  nc = len(wl[0])
  wlw.setPowError(epow)
  ...
  for i,c in enumerate(curves):
    ...
    fs = wlw.replaceNulls(fs,freplace)
    gs = wlw.replaceNulls(gs,freplace)
    fmed = wlw.replaceNulls(fmed,freplace)
    if c=="p":
      fs = mul(fs,100)
      gs = mul(gs,100)
      fmed = mul(fmed,100)
      fmad = mul(fmad,100)
    #fmad = abs(sub(fmed,gs))
    sp = SimplePlot(SimplePlot.Origin.UPPER_LEFT)
    sp.setSize(500,900) # CWPposter
    sp.setVLabel("Depth (km)")
    sp.setHLabel("Log index")
    ...
    #sp.setFontSizeForSlide(1.0,1.0,16.0/9.0)
    #sp.paintToPng(720.0,3.08,c+"before.png")
    sp.setFontSizeForPrint(8.0,187.6)
    #sp.paintToPng(720.0,2.6,_pngDir+c+"before.png")
    #sp.paintToPng(720.0,3.72,_pngDir+c+"before2figs.png")
    ...
    #sp.paintToPng(720.0,3.08,c+"after.png")
    sp.setFontSizeForPrint(8.0,187.6)
    #sp.paintToPng(720.0,3.72,_pngDir+c+"afterDonly2figs.png")
    #sp.paintToPng(720.0,3.72,_pngDir+c+"afterStatic.png")
    #sp.paintToPng(720.0,2.6,_pngDir+c+"afterDonly.png")
    #sp.paintToPng(720.0,2.6,_pngDir+c+"after.png")

    sp = SimplePlot(SimplePlot.Origin.UPPER_LEFT)
    sp.setSize(500,900) # CWPposter
    sp.setVLabel("Relative geologic time")
    sp.setHLabel("Log index")
    sp.addColorBar("Median "+ltype+units)
    sp.plotPanel.setColorBarWidthMinimum(90)
    pv = sp.addPixels(sz,Sampling(nl,1,1),fmed)
    pv.setInterpolation(PixelsView.Interpolation.NEAREST)
    pv.setColorModel(ajet)
    pv.setClips(fclips[0],fclips[1])
    #sp.setFontSizeForPrint(12.0,222) #GP newsletter
    ...
    #sp.paintToPng(720.0,3.08,c+"after.png")
    sp.setFontSizeForPrint(8.0,187.6)
    #sp.paintToPng(720.0,2.6,_pngDir+c+"medDonly.png")
    #sp.paintToPng(720.0,2.6,_pngDir+c+"med.png")

    sp = SimplePlot(SimplePlot.Origin.UPPER_LEFT)
    sp.setSize(500,900) # CWPposter
    sp.setVLabel("Relative geologic time")
    sp.setHLabel("Log index")
    sp.addColorBar("|"+ltypec+"- median "+ltype+"|")
    sp.plotPanel.setColorBarWidthMinimum(90)
    pv = sp.addPixels(sz,Sampling(nl,1,1),fmad)
    pv.setInterpolation(PixelsView.Interpolation.NEAREST)
    pv.setColorModel(ajet)
    pv.setClips(mclips[0],mclips[1])
    #sp.setFontSizeForPrint(12.0,222) #GP newsletter
    #sp.setFontSizeForSlide(1.0,1.0,16.0/9.0)
    #sp.paintToPng(720.0,3.08,c+"after.png")
    sp.setFontSizeForPrint(8.0,187.6)
    #sp.paintToPng(720.0,2.6,_pngDir+c+"madDonly.png")
    #sp.paintToPng(720.0,2.6,_pngDir+c+"mad.png")
  print "Average MAD: ",mdevs/nc
  """
  sp1 = SimplePlot(SimplePlot.Origin.UPPER_LEFT)
  #sp1.setSize(650,550)
  sp1.setSize(800,500) # CWPposter
...
def goSort():
  ...
  #pv.setMarkSize(6)
  """
  x = [x[127],x[125],x[124],x[147],x[149],
       x[150],x[152],x[168]] #iline
  y = [y[127],y[125],y[124],y[147],y[149],
       y[150],y[152],y[168]] # iline
  """
  """
  x = [x[141],x[ 80],x[124],x[125],x[130],x[159]] # xline
  y = [y[141],y[ 80],y[124],y[125],y[130],y[159]] # xline
  pv = sp.addPoints(x,y)
  ...
  pv.setMarkSize(10)
  """
  x = [x[ 80],x[ 94],x[124],x[127],
        x[130],x[141],x[150],
        x[152],x[159],x[168],x[125],x[149],x[147]]
  y = [y[ 80],y[ 94],y[124],y[127],
        y[130],y[141],y[150],
        y[152],y[159],y[168],y[125],y[149],y[147]]
  ...
  sp.setFontSizeForPrint(8.0,222.0)
  #sp.paintToPng(720.0,3.08,_pngDir+"xsectionLine.png")
  #sp.paintToPng(720.0,3.08,_pngDir+"logpairs.png")
...
def goUnconformityTest():
  ...
  for pair in pairs:
    ia,ib = pair[0],pair[1]
    e = wlw.computeErrors(0,f[ia],f[ib])
    wlw.interpolateOddErrors(e)
    nl,nk = len(e[0]),len(e)
    lmax = (nl-1)/2
    lmin = -lmax
...
_pngDir = "/Users/lwheeler/Documents/01_School/01_Grad/04_Research/png/raw/"
...
def writeFile(c,va):
  nz = len(va[0])
  nl = len(va)
  ofile = open('../data/'+c+'logs.txt','r+')
  ofile.write('z'     +'\t'+'log 1' +'\t'+'log 2' +'\t'+
              'log 3' +'\t'+'log 4' +'\t'+'log 5' +'\t'+
              'log 6' +'\t'+'log 7' +'\t'+'log 8' +'\t'+
              'log 9' +'\t'+'log 10'+'\t'+'log 11'+'\t'+
              'log 12'+'\t'+'log 13'+'\t'+'\n')
  ...
  for z in range(nz):
    ofile.write(str(z)+'\t'+str(va[0][z])+'\t'+str(va[1][z])+'\t'+
                ...
                str(va[5][z])+'\t'+str(va[6][z])+'\t'+str(va[7][z])+'\t'+
                str(va[8][z])+'\t'+str(va[9][z])+'\t'+str(va[10][z])+'\t'+
                str(va[11][z])+'\t'+str(va[12][z])+'\t'+'\n')
  ofile.close()
