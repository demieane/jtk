...
public class WellLogWarpingTest {
  ...
  private void computeWeights(float[] wm, float[][][] wl, Pairs[] ps, int[] ls) {
    ...
    for (int lp=0; lp<nlp; ++lp) {
      ...
      ws = tae>0 ? np*np/tae : 1.0f; 
      //ws = tae>0 ? np*pow(np/tae,2.0/_epower[m]) : 1.0f; 
      ps[lp].ws = ws;
      wsum[m] += ws;
      ls[il] += 1;
      ls[jl] += 1;
        ...
        for (int im=0; im<nm; ++im) {
          ...
          if (wm[im]>0 && wellNotNull(fi) && wellNotNull(gj)) {
            ...
            if (np>0) {
              ...
              float[] zj = zij[1];
              dump(sub(zj,zi));
              pt[nij.getAndIncrement()] = new Pairs(il,jl,im,is,js,zi,zj,np);
            }
          }
        }
      }
 ...
 private static float[][] makeZs(int[][] ij) {
   int ni = ij.length;
   int np = ij[0].length;
   float[][] zij = new float[ni][np];
   int nlo,nhi;

   for (int ii=0; ii<ni; ++ii) {
     zij[ii][0] = ij[ii][0];
     for (int ip=1; ip<np; ++ip) {
       if(ij[ii][ip]-ij[ii][ip-1]>0) {
         zij[ii][ip] = ij[ii][ip];
         if (ii==0)
           zij[ii+1][ip] = ij[ii+1][ip];
         else
           zij[ii-1][ip] = ij[ii-1][ip];
       } else {
         nlo = nhi = ip;
         while(nlo>0 && ij[ii][nlo]-ij[ii][nlo-1]==0)
           --nlo;
         while(nhi<np-1 && ij[ii][nhi]-ij[ii][nhi-1]==0) // treating this correctly? what if i or j is const up to last sample?
           ++nhi;
         float dj = 1.0f/(nhi-nlo);
         zij[ii][ip] = dj*((nhi-ip)*ij[ii][nlo]+(ip-nlo)*ij[ii][nhi]);
         if (ii==0)
           zij[ii+1][ip] = ij[ii+1][ip];
         else
           zij[ii-1][ip] = ij[ii-1][ip];
  ...
  private static class Pairs {
    Pairs(int il, int jl, int m, int[] is, int[] js, float[] zi, float[] zj, int np)
  {
      this.il = il;
      this.jl = jl;
      this.is = is;
  ...
  private static class A implements CgSolver.A {
    A(float[][] tz, int[][] c, double wp,int np, Pairs[] ps) {
    ...
    */
    ...
    private void testNND(int nk, int nl) {
      ...
      float[][] yt = copy(y);
      applyLhs(_tz,_tc,_wp,_np,_ps,copy(y),y);
    ...
    for (int ip=0; ip<np; ++ip) { // for all log pairs, ...
      ...
      for (int k=0; k<n; ++k) { // for all index pairs (i,j), ...
        ...
        int jt = (int)(tj[jk]+0.5f);
          float dif = jz-iz;
        if (0<it && it<nt && 0<jt && jt<nt && abs(dif)%1==0) {
  ...
  private static void applyLhs(
    float[][] tz, int[][] c, double wp,int npp, Pairs[] ps, float[][] x, float[][] y) 
  {
    ...
    for (int ip=0; ip<np; ++ip) { // for all log pairs, ...
      ...
      for (int k=0; k<n; ++k) { // for all index pairs (i,j), ...
        ...
        int it = (int)(ti[ik]+0.5f);
        int jt = (int)(tj[jk]+0.5f);
        if (0<it && it<nt && 0<jt && jt<nt && c[il][it]>0 && c[jl][jt]>0) {
          float scl = (float)(ws*ws);
    ...
    constrainSmoothShifts(wp,npp,c,x,y);
  }
  ...
  public static void invertZorT(float[][] t) {
    int n1 = t[0].length;
    int n2 = t.length;
    float[] ut = rampfloat(0.0f,1.0f,n1);
    float[] uz = rampfloat(0.0f,1.0f,n1);
    float[] z = zerofloat(n1);
    for (int i2=0; i2<n2; ++i2) {
      cleanZorT(t[i2]);
      interpolate(t[i2],uz,ut,z); 
      copy(z,t[i2]);
    }
  }
...
*/
  ...
  /*
  public void testSymmetryOfDW(float[] fi, float[] gj) {
    int nk = fi.length;
...
*/ 
     ...
     Color[] col = {Color.black,Color.blue,Color.green,Color.magenta,
                    Color.red,Color.orange};
     int nc = col.length;
     PlotPanel pp = new PlotPanel(PlotPanel.Orientation.X1DOWN_X2RIGHT);
     float[] d = rampfloat(0.0f,1.0f,nt);
     PointsView pv = pp.addPoints(d,r[0]);
     pv.setLineColor(col[0]);
     pv.setLineWidth(1.2f);
     for (int il=1; il<nl; ++il) {
       pv = pp.addPoints(d,r[il]);
       pv.setLineColor(col[il-nc*(il/nc)]);
       pv.setLineWidth(1.2f);
     }
     pp.setTitle("it="+iter);
     PlotFrame pf = new PlotFrame(pp);
...
*/ 
  ...
  private int sumPairs(float[][] tz, Pairs[] ps) {
    int nt = tz[0].length;
    int np = ps.length; // number of log pairs
    int npp = 0;
    for (int ip=0; ip<np; ++ip) { // for all log pairs, ...
    ...
    int iter, float[] s, int[] ls, Pairs[] ps, float[][] tz, float[][] q) 
  {
   ...
   float sigma = 100.0f;
   float small = 1.0e-6f;
   int niter = 300*iter+300;
   //niter = niter>2000 ? 2000 : niter;
   int[][] c = new int[nl][nt]; // for right-hand side
   double wp = sumWeights(tz,ps);
   int npp = sumPairs(tz,ps);
   A a = new A(tz,c,wp,npp,ps);
   ...
   makeRhs(tz,c,s,ps,b);
   applyLhs(tz,c,wp,npp,ps,copy(p),p);
   sub(b,p,b);
  ...
  private static void constrainSmoothShifts(
    double wp, int np, int[][] c, float[][] x, float[][] y) 
  {
    int nt = x[0].length; // number of depths
    int nl = x.length; // number of logs
    //wp *= 0.3;
    ...
    for (int il=0; il<nl; ++il) {
      ...
      float[] yi = y[il];
      for (int it=1; it<nt; ++it) { 
        //double wk = c[il][it]>0 ? 0.0d : wp;
        double wk = (c[il][it]==0 || c[il][it-1]==0) ? wp : 0.0d;
        //double wk = (c[il][it]==0 || c[il][it-1]==0) ? wp*0.1 : wp/np*nl;
        //double wk = (c[il][it]==0 || c[il][it-1]==0) ? wp : nl/(1.0d+c[il][it]+c[il][it-1]);
  ...
  public void subtractMeanOverLogs(int[] ls, float[][] x) {
    int nk = x[0].length;
    int nl = x.length;
    int cl = 0;
